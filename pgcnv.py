...
def main():
    ...
    output_file = f'{output_path}/pgcnv_res_{current_datetime}.csv'
    ...
    input_dim = X.shape[1]
    hidden_dim = int(paths['hidden_dim'])
    output_dim = 1

    model = GCN(input_dim, hidden_dim, output_dim)

    optimizer = torch.optim.Adam(model.parameters(), lr=float(paths['learning_rate']))
    loss_fn = nn.BCEWithLogitsLoss()  # Binary Cross Entropy Loss

    num_epochs = int(paths['hidden_dim'])
    ...
    print(f"Predicted labels have saved to the '{output_file}' file")
...
if __name__ == '__main__':
    ...
    et = time.time()
    rt = et - st
    print(f"step8 finish! runtime: {rt}sec")
